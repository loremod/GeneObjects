...
test_that("CodingGene class functionality", {
  ...
  gene <- CodingGene(id = "ENSG00000139618", symbol = "BRCA2", name = "BRCA2",
                     ...
                     struct = structure, proteinID = "P51587", proteinSequence = AAString("MENQWEK"))
  ...
  expect_equal(as.character(productSequence(gene)), "MENQWEK")
  expect_true(is(productSequence(gene), "AAString"))
  expect_equal(lengthProduct(gene), 7)
...
test_that("LncRNAGene class functionality", {
  ...
  gene <- LncRNAGene(id = "ENSG00000273105", symbol = "MALAT1", name = "MALAT1",
                     ...
                     struct = structure, lncRNAID = "NONHSAG000347", RNASequence = "GAUUACA")
  ...
  expect_true(is(struct(gene), "GeneStructure"))
  expect_equal(as.character(productSequence(gene)), "GAUUACA")
  expect_true(is(productSequence(gene), "RNAString"))
  expect_equal(lengthProduct(gene), 7)
...
test_that("MicroRNAGene class functionality", {
  ...
  gene <- MicroRNAGene(id = "ENSG00000284458", symbol = "MIR21", name = "MicroRNA 21",
                       ...
                       microRNAID = "MI0000077", microRNASeedSequence = RNAString("UAGCUU"))
  ...
  expect_true(is(struct(gene), "GeneStructure"))
  expect_equal(as.character(productSequence(gene)), "UAGCUU")
  expect_true(is(productSequence(gene), "RNAString"))
...
test_that("RibosomalRNA class functionality", {
  ...
  gene <- RibosomalRNA(id = "ENSG00000175899", symbol = "RNA5-8S5", name = "RNA, 5.8S ribosomal",
                       ...
                       rRNAID = "NR_023363", rRNASequence = RNAString("ACCUC"))
  ...
  expect_true(is(struct(gene), "GeneStructure"))
  expect_equal(as.character(productSequence(gene)), "ACCUC")
  expect_true(is(productSequence(gene), "RNAString"))
  expect_equal(lengthProduct(gene), 5)
  expect_equal(numExons(gene), 2)
})
...
test_that("SmallNuclearRNA class functionality", {
  ...
  expect_true(is(struct(gene), "GeneStructure"))
  expect_equal(as.character(productSequence(gene)), "GGGCC")
  expect_true(is(productSequence(gene), "RNAString"))
  expect_equal(lengthProduct(gene), 5)
  expect_equal(numExons(gene), 2)
})
...
test_that("PiwiInteractingRNA class functionality", {
  ...
  gene <- PiwiInteractingRNA(id = "ENSG00000284332", symbol = "PIWIL4", name = "Piwi like RNA",
                             ...
                             piRNAID = "DQ571952", piRNASequence = "CUGCUG")
  ...
  expect_true(is(struct(gene), "GeneStructure"))
  expect_equal(as.character(productSequence(gene)), "CUGCUG")
  expect_true(is(productSequence(gene), "RNAString"))
  expect_equal(lengthProduct(gene), 6)
  expect_equal(numExons(gene), 2)
})
